...
func solve() {
	...
	// 各薬品単体から開始可能かチェック
	for i := 0; i < n; i++ {
		singleDrug := 1 << i // 薬品 i+1 のみの状態
		if isSafe(singleDrug) {
			visited[singleDrug] = true
			queue = append(queue, singleDrug)
		}
	}
	...
	for len(queue) > 0 {
		...
		for i := 0; i < n; i++ {
			drugBit := 1 << i
			// 薬品 i+1 がまだ含まれていない場合
			if (current & drugBit) == 0 {
				nextState := current | drugBit
				if !visited[nextState] && isSafe(nextState) {
