...
func main() {
	...
	for i := 1; i <= n; i++ {
		// モンスターiを倒した時の魔力を使ったか使ってないかで体力のmaxを取る
		for j := 1; j <= m; j++ {
			tukawanai := dp[i-1][j] - abes[i][1]
			tukau := -1
			if j+abes[i][2] <= m {
				tukau = dp[i-1][j+abes[i][2]]
			}
			dp[i][j] = max(tukawanai, tukau)
		}
	}
...
func nisWithOffset(n, offset int) []int {
	a := make([]int, n+offset)
