...
func main() {
	...
	min := math.MaxInt32
	var dfs func(cur, cnt int, graph [][]bool)
	dfs = func(cur, cnt int, graph [][]bool) {
		deb(cur, cnt, graph)
		// 最小手数を更新
		valid := true
		for i := 1; i <= n; i++ {
		...
		for i := cur+1; i <= n; i++ {
			...
			ng2 := make([][]bool, n+1)
			for i := range graph {
				ng1[i] = make([]bool, len(graph[i]))
				copy(ng1[i], graph[i])
				ng2[i] = make([]bool, len(graph[i]))
				copy(ng2[i], graph[i])
			}
			// iへの辺が貼ってある場合
			if g[cur][i] {
...
func deb(cur, cnt int, graph [][]bool) {
	fmt.Println("================")
	fmt.Print("cur: ")
	fmt.Print(cur)
	fmt.Print(",cnt: ")
