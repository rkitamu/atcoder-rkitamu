...
class Program
{
    static void Main()
    {
        ...
        var grid = StdReader.ReadMatrix<int>(n, 3);
        Array.Sort(grid, (a, b) => a[0].CompareTo(b[0]));
        var prevt = 0;
        var prevx = 0;
        var prevy = 0;
        for (int i = 0; i < n; i++)
        {
            var t = grid[i][0] - prevt;
            var x = Math.Abs(grid[i][1] - prevx);
            var y = Math.Abs(grid[i][2] - prevy);
