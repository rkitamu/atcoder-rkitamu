...
class Program
{
    static void Main()
    {
        var N = Ri();
        var D = Ri();
        var dayPrices = new Dictionary<int, List<int>>();
        for (int i = 0; i < N; i++)
        {
            var day = Ri();
            var price = Ri();
            if (!dayPrices.ContainsKey(day))
            {
                dayPrices[day] = new List<int>();
            }
            dayPrices[day].Add(price);
        }
        ...
        for (int day = 1; day <= D; day++)
        {
            if (dayPrices.ContainsKey(day))
            {
                foreach (var price in dayPrices[day])
                {
                    pq.Enqueue(price, price);
                }
            }
            if (pq.Count > 0)
            {
