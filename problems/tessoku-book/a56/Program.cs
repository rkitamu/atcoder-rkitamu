...
class Program
{
    static void Main()
    {
        ...
        while (Q-- > 0)
        {
            ...
            var c = Ri();
            var d = Ri();
            var hun = new StaticModInt<Mod1000000007>(100);
            var hun2 = new StaticModInt<Mod1000000007>(100);
            var hash1 = sHash[b] - sHash[a - 1] * hun.Pow(b - a + 1);
            var hash2 = sHash[d] - sHash[c - 1] * hun2.Pow(d - c + 1);
            if (hash1 == hash2)
            {
                WYes();
            }
