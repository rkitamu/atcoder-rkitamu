...
class Program
{
    static void Main()
    {
        ...
        var owari = new List<int>();
        var sippai = new List<int>();
        ...
        for (int i = 0; i < N; i++)
        {
            ...
            {
                if (!trie[curr].ContainsKey(c))
                {
                    trie[curr][c] = trie.Count;
                    trie.Add(new Dictionary<char, int>());
                    owari.Add(0);
                    sippai.Add(0);
                }
        ...
        while (q.Count > 0)
        {
            ...
            foreach (var pair in trie[u])
            {
                ...
                int f = sippai[u];
                while (f != 0 && !trie[f].ContainsKey(c))
                {
                    f = sippai[f];
                }
                sippai[v] = trie[f].ContainsKey(c) ? trie[f][c] : 0;
                owari[v] |= owari[sippai[v]];
                q.Enqueue(v);
            }
        }

        // 文字列の長さi 状態j, ビットマスク でdpして全部の状態の値を取れば良い
        var dp = new AtCoder.StaticModInt<Mod998244353>[L + 1, trie.Count, 1 << N];
        dp[0, 0, 0] = 1;

        for (int i = 0; i < L; i++)
        {
            for (int j = 0; j < trie.Count; j++)
            {
                for (int mask = 0; mask < (1 << N); mask++)
                {
                    if (dp[i, j, mask].Value == 0) continue;

                    for (char c = 'a'; c <= 'z'; c++)
                    {
                        int state = j;
                        while (state != 0 && !trie[state].ContainsKey(c))
                        {
                            state = sippai[state];
                        }
                        if (trie[state].ContainsKey(c))
                        {
                            state = trie[state][c];
                        }

                        int nextMask = mask | owari[state];
                        dp[i + 1, state, nextMask] += dp[i, j, mask];
                    }
                }
            }
        }
        ...
        //TODO: 計算
        AtCoder.StaticModInt<Mod998244353> ans = 0;
        int all = (1 << N) - 1;
        for (int j = 0; j < trie.Count; j++)
        {
            ans += dp[L, j, all];
