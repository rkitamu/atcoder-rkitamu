...
func main() {
	...
	c := make(map[Pair[int, int]]int)
	xyes := nis2d(n, 2, 0)
	sort.Slice(xyes, func(i, j int) bool {
		if xyes[i][0] == xyes[j][0] {
			return xyes[i][1] < xyes[j][1]
		}
		return xyes[i][0] < xyes[j][0]
	})
	for i := 0; i < n; i++ {
		for j := i + 1; j < n; j++ {
			x := xyes[j][0] - xyes[i][0]
			...
			m[NewPair(xd, yd)]++
			c[NewPair(xyes[j][0] + xyes[i][0], xyes[j][1] + xyes[i][1])]++
		}
	}
