...
func main() {
	...
	ans := make([][]byte, 0)
	var restore func(t3 Query)
	restore = func(t3 Query) {
		idx--
		// サーバー反映元のPCに入っていた文字列を復元する
		pc := t3.p
		for 0 <= idx {
			if queries[idx].p == pc {
				if queries[idx].t == 1 {
					// PCの文字の最初はここ時点のサーバの文字列が使用される
					for 0 <= idx {
						if queries[idx].t == 3 {
							restore(queries[idx])
							return
				...
				} else if queries[idx].t == 2 {
					if queries[idx].p == pc {
						ans = append(ans, []byte(queries[idx].s))
					}
				} else if queries[idx].t == 3 {
					// t1が来ない限り最終的には現在処理しているt3のクエリが反映されるため何もしない
				}
			}
			idx--
		}
